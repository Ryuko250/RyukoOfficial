...
function getPackageFromURL() {
  ...
  if (whatsappEl) {
    if (whatsapp) {
      ...
      whatsappEl.parentElement.style.display = "flex";
    } else {
      whatsappEl.parentElement.style.display = "none";
    }
  }
  
  return { product, items, amount, userId, zoneId, server, whatsapp };
}
...
if (confirmBtn) {
  confirmBtn.onclick = async () => {
    ...
    const { product, items, amount, userId, zoneId, server,whatsapp } = getPackageFromURL();
    ...
    if (whatsapp) msg += `📞 WhatsApp Number: ${whatsapp}\n`;
