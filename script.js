...
const result = document.getElementById("mlbbResult");

if(!userId || !zoneId){
  result.innerHTML = "⚠ Enter User ID and Server ID";
  return;
}

result.innerHTML = "🔎 Checking player...";

try{

const response = await fetch(`https://mlbb-api.ridwaanhall.repl.co/check-username${userId}/${zoneId}`);
...
result.innerHTML = `
<div class="mlbb-result-box">
<b>🎮 Game Name:</b> ${data.nickname} <br>
<b>🆔 User ID:</b> ${userId} <br>
<b>🌍 Server ID:</b> ${zoneId} <br>
<b>🌎 Country:</b> ${data.region || "Unknown"}
</div>
`;

}else{

result.innerHTML = "❌ Player not found. Check ID or Server.";
...
console.error(error);
result.innerHTML = "⚠ API error or server blocked request.";
